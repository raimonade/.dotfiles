...
test("blocks shell commands that definitely mutate worker configuration", () => {
	const commands = [
		...
		`rm -f ${PROTECTED_FILE}`,
		`echo ok & rm ${PROTECTED_FILE}`,
		`/bin/rm ${PROTECTED_FILE}`,
		`command rm ${PROTECTED_FILE}`,
		`>/dev/null rm ${PROTECTED_FILE}`,
		`git restore ${PROTECTED_FILE} && git status --short`,
		`git checkout -- ${PROTECTED_FILE}`,
		`git rm ${PROTECTED_FILE}`,
		`mv ${PROTECTED_FILE} /tmp/worker-configuration.d.ts`,
		`mv /tmp/generated.d.ts ${PROTECTED_FILE}`,
