...
Personal defaults for every project. Local repo `AGENTS.md` always wins. (Pi
workspace structure lives in `~/.pi/AGENTS.md`.) Canonical cross-agent
instructions — read before working: `~/.agents/POLICY.md` (what / how much to
build), `~/.agents/CONDUCT.md` (how to work), the installed `coding-standards`
skill (before non-trivial TypeScript work), `~/.agents/EXECUTOR.md`
(reaching integrations — Mobbin, Linear, Axiom, … — via the `executor` CLI).
Mobbin default: when using Mobbin via Executor/MCP, ask for latest/current/recent
screenshots and prefer newest-looking results; rerun if results look stale. The
two blocks below are always-on subsets of those, kept in sync via markers.
...
- **Plannotator:** proactively offer `plannotator annotate <path>` for non-trivial plans with user choices/tradeoffs/phases, and `plannotator review --git` for substantial diffs before handoff. Keep tiny/routine/urgent work inline; offer rather than block unless approval gating was requested.
...
- **Delivery invariants** (full list in `~/.agents/CONDUCT.md`): confirm the authoritative artifact before porting anything found on disk; a named reference means exact parity with file:line, deviations are questions; ship the visible outcome before plumbing; "done" is an observed live check or screenshot, else report unverified; resolve collective nouns (backlog, the app) to a concrete list before acting or spawning; state prod vs dev before diagnosing; narrate one line per step and never idle after a background job finishes; on "continue"/limits reset/compaction restate next item + proof + stop condition and carry standing rules forward.
- **Parallel lanes:** only over independent tickets, one owner per lane, one worktree per branch, no shared files, max six concurrent, typecheck/CI serialized; coupled problems (red main, migration, design call) get one owner; check the roster so nothing runs twice.
...
- **Customer material never enters a repo:** files a customer supplied, figures extracted from them, rate cards, audience data, email evidence, account/organization identifiers, and customer-named scripts, branches or PR text stay OUTSIDE the working tree (or in an ignored path) — never committed, pushed, or pasted into a PR body/commit message. Keep working notes outside the repo and reference them by local path. If such material is already committed, remove it in its own PR immediately and state plainly that the blobs remain in history until a rewrite or provider purge.
...
For open-ended code navigation, call/impact analysis, cross-project context, or past-session recovery, load the `tracedecay` skill before broad grep/file reading when `~/.tracedecay/daemon.sock` exists. If the socket is absent, the daemon is intentionally contained: do not start or reinstall it; use normal tools instead. The Pi PATH wrapper blocks daemon startup while `~/.pi/agent/state/tracedecay-daemon-contained` exists; never bypass it with the absolute Homebrew binary.

## Frontend/design model routing

Execution models are not the default taste models. For taste-heavy frontend work, use Claude Code CLI with Opus for design direction, visual implementation delegation when appropriate, and final visual review. Never use Fable — not as the session model and never as a subagent/delegate model. Do not downgrade design direction or review to Sonnet.
...
- Run `claude -p` directly and synchronously in the foreground. Herdr is not required; use Herdr only when the user explicitly requests an interactive Herdr session. An unset `HERDR_ENV` says nothing about Claude availability. Treat Opus as unavailable only when the foreground command reports a missing binary, authentication failure, or model-access failure.
- Pin `--model 'claude-opus-5[1m]'`; never pass the floating `opus` alias because it may resolve to Opus 4.8.
...
- If Claude Opus is unavailable/unauthenticated, say so and pause instead of shipping taste-heavy UI unaided.
- Verify user-facing UI with screenshot/browser review when possible; for anything visually meaningful, request/perform a Claude Opus review before final handoff.
