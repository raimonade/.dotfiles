...
description: Adopt better-result in an existing TypeScript codebase. Use when a TypeScript change touches expected failure design, parse/validation error flows, try/catch, Promise rejection handling, null sentinels, thrown domain exceptions, or Result-returning APIs in a codebase that uses or is adopting better-result.
references:
