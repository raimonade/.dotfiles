---
name: elements
description: "Installs and customizes assistant-ui elements, the styled catalog at assistant-ui.com/elements via `npx assistant-ui@latest add <item>`. Use when adding a prebuilt chat surface (Thread, ThreadList, AssistantModal, AssistantSidebar, ToolFallback, ToolGroup, MarkdownText, and related widgets) or a standalone element, choosing runtime-connected `.aui.tsx` vs props-driven files, overriding Thread `components`, editing copied source under `components/assistant-ui/elements/`, or picking Radix vs Base UI in `components.json`. Route here when `@/components/assistant-ui/...` fails, an element is unstyled, or the CLI installs the wrong flavor. For unstyled blocks use primitives; for CLI scaffold use setup."
license: MIT
---
