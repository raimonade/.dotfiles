---
name: primitives
description: "Builds custom assistant-ui chat UI from unstyled `@assistant-ui/react` primitives (Radix-style parts: ThreadPrimitive, ComposerPrimitive, MessagePrimitive, ActionBarPrimitive, BranchPickerPrimitive, AttachmentPrimitive, and related). Use when assembling a custom Thread, Composer, message list, action bar, mention/slash popover, or suggestion grid; when wiring composer input, dictation, quoting, or attachments; or when handling editing, branching, virtualization, or grouped chain-of-thought UI. Covers `MessagePrimitive.Parts` renderers, `groupPartByType` (`standalone-tool-call` in 0.15), `AuiIf` selectors, and wrapping in `AssistantRuntimeProvider` (primitives are unstyled). For drop-in UI use setup; for the styled catalog use elements; for thread lists use thread-list."
license: MIT
---
