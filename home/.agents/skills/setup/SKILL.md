---
name: setup
description: "Installs and configures assistant-ui with the CLI (`create`, `init`, `add`, `update`, `upgrade`, `codemod`, `doctor`) and picks a runtime adapter. Use for first-time `npx assistant-ui@latest create` (templates `default`, `minimal`, `cloud`, `langchain`, `mcp`, `eve`, plus `--example` scaffolds), `init` on existing Next.js, or keeping an install current. Covers runtime hooks (`useChatRuntime`, `useLangGraphRuntime`, `useStreamRuntime`, `useAdkRuntime`, `useA2ARuntime`, `useAgUiRuntime`, `useLocalRuntime`, `useExternalStoreRuntime`, `useDataStreamRuntime`, `useAssistantTransportRuntime`) and Expo/Ink scaffolds. Route here for the CLI, choosing a runtime, or a platform target; then elements for styled Thread, primitives for unstyled blocks, runtime for adapters in depth, and update for migrations."
license: MIT
---
