---
name: runtime
description: "Guide to the assistant-ui runtime: the state and action layer in `@assistant-ui/react`. Use when creating a runtime (`useLocalRuntime`, `useExternalStoreRuntime`, `useRemoteThreadListRuntime`, `useCloudThreadListRuntime`, `useAssistantTransportRuntime`), wiring `AssistantRuntimeProvider`, or reading thread, message, composer, and attachment state via `useAui`, `useAuiState`, `useAuiEvent`. Covers scope accessors (`aui.thread`, `aui.threads`, `aui.message`, `aui.composer`), `AuiConfig`/`AuiProvider`, `threads.selectionChanged`, RuntimeCapabilities, and attachment/speech/history adapters. Route here for provider errors, removed v0.12 hooks, object-literal selector re-renders, or stale state. For conversation list UI use thread-list; for toolkits use tools."
license: MIT
---
