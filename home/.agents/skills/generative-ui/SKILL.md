---
name: generative-ui
description: "Lets the model compose UI at runtime from a component vocabulary instead of one component per tool. Use for the present tool: `JSONGenerativeUI`, `defaultGenerativeUILibrary`, and `present({ display })` from `@assistant-ui/react-generative-ui` via `AuiConfig({ tools: Tools({ toolkit }) })`. Also covers `MessagePrimitive.GenerativeUI` allowlists, Slack Block Kit and Teams Adaptive Cards, A2UI over AG-UI, and OpenUI. Route here when the model should design layout, an unknown `$type` throws or renders nothing, or a tree must post to Slack/Teams or paint A2UI. For a widget tied to one known tool call, use tools instead."
license: MIT
---
