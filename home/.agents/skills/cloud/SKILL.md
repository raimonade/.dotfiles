---
name: cloud
description: "Adds AssistantCloud persistence, auth, and telemetry to assistant-ui. Use when wiring cross-session thread/message history, multi-device chat, feedback, file uploads, or auth: `cloud` on `useChatRuntime`, `AISDKThreads({ cloud })`, `useCloudChat`/`useThreads` from `@assistant-ui/cloud-ai-sdk`, or `cloud` on `useLangGraphRuntime`. Covers `AssistantCloud` with JWT `authToken`, server `apiKey` plus `userId`/`workspaceId`, or `anonymous`; Clerk, Auth0, Supabase, Firebase; threads/messages/files/runs/telemetry; `ThreadHistoryAdapter` via `CloudMessagePersistence`; and `NEXT_PUBLIC_ASSISTANT_BASE_URL`/`ASSISTANT_API_KEY`. Route here for threads that do not persist, cloud 401s, or feedback that does not save. For `<ThreadList />` use thread-list; for adapter contracts use runtime."
license: MIT
---
