---
name: assistant-ui
description: "Overview and router for assistant-ui, the React library for AI chat UIs from unstyled primitives and a styled elements catalog. Use for high-level, cross-cutting, or architecture questions: choosing packages, picking a runtime, or understanding layers (elements, primitives, AuiConfig/AuiProvider, runtime, adapters) and the message model. Covers `@assistant-ui/react` 0.15.x, `@assistant-ui/ai-sdk` for AI SDK v7 (`useChatRuntime`, `AssistantChatTransport`), `@assistant-ui/core`, `@assistant-ui/store`, `assistant-stream`, `assistant-cloud`, adapters (LangGraph, LangChain, Google ADK, A2A, AG-UI, Eve, OpenCode, Pi), and `@assistant-ui/react-native` plus `@assistant-ui/react-ink`. For a specific area route to a sibling: setup, elements, primitives, runtime, tools, generative-ui, streaming, cloud, thread-list, copilots, markdown, react-mcp, observability, react-native, ink, or update."
license: MIT
---
