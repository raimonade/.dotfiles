---
name: tools
description: "Defines model-callable tools and their call UI in assistant-ui. Covers `\"use generative\"` toolkits via `defineToolkit`, compiled by `withAui` (Next/Metro) or `aui()` (Vite), mounted with `AuiConfig({ tools: Tools({ toolkit }) })`, and exposed via `AISDKToolkit`. Covers tool kinds (`humanTool`, `providerTool`, `externalTool`, `stubTool`, `defineMcpToolkit`), render/`toModelOutput`, approval gates, MCP Apps, WebMCP, and sub-agent messages. Reach for it when a tool is never called, tool UI does not render, a frontend result never reaches the model, `humanTool()` throws, or an approval gate cannot be answered. For model-composed vocabulary UI use generative-ui; for user-added browser MCP use react-mcp; for styled ToolFallback/ToolGroup use elements."
license: MIT
---
