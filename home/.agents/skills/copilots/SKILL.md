---
name: copilots
description: "Ground an assistant in the app with assistant-ui copilots (`@assistant-ui/react`). Use for `useAssistantInstructions`, lazy send-time state via `useAssistantContext({ getContext })`, `makeAssistantVisible`, interactables (`unstable_useInteractable`, `unstable_interactableTool`, `AuiConfig({ unstable_interactables: unstable_Interactables() })`), `aui.modelContext.register`, or iframe bridging with `AssistantFrameProvider`/`useAssistantFrameHost`. Legacy `Interactables()`, `useAssistantInteractable`, and `useInteractableState` are deprecated; new code uses the unstable_ API. Reach for this when the assistant should read the page, click or edit UI, update component state, or receive tools from a sandboxed iframe. For LLM tool-call UI use tools; for thread state use runtime."
license: MIT
---
