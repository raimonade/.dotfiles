---
name: streaming
description: "Streaming wire protocols for assistant-ui via `assistant-stream`. Use when building a custom streaming endpoint (not Vercel AI SDK) with `createAssistantStreamResponse`, `createAssistantStream`, or `createAssistantStreamController`; choosing Data Stream (`useDataStreamRuntime`, `DataStreamEncoder`) vs Assistant Transport (`AssistantTransportEncoder`, `useAssistantTransportRuntime`); decoding with `AssistantStream.fromResponse`; or wiring resumable streams (`assistant-stream/resumable`). Route here for unexpected part-start/text-delta shapes, a tool call that never settles, dropped source/file parts, Content-Type mismatch, or a reload that cannot resume. For runtime hook state use runtime; for AI SDK `useChatRuntime` scaffolding use setup; for persistence use cloud."
license: MIT
---
